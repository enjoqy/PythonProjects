...
def get_one_page(url, province_name):
    ...
    for el in els:
        ...
        if not len(jop_req) == 0:
            ...
            jop_reqs = ''
        row = [job_title, company_name, working_place, salary, release_time]
        row = row + jop_reqs
        print(row)
        rows.append(row)
...
#  将爬虫的数据写入到csv格式文件中
def write(rows, province_name):
    with open('H://province_jobs/company_51job_cloudcom/' + province_name + '.txt', mode="a+", encoding="utf-8", newline="") as f:
...
def connect_DB(row, province_name):
    ...
    cursor = db.cursor()
    # 执行插入操作
    sql = "INSERT INTO company_51job_ui (province, job_title, company_name, working_place, salary, release_time, district, work_exp, degree, hiring_number) " \
          " VALUES " \
